...
Simus <- mclapply(1:100, function(lalal){
  loc             <- paste0("loc", 1:20)
  Loc_T           <- matrix(rnorm(n * p * 4), ncol = p * 4)
  colnames(Loc_T) <- paste(rep(loc, each = 4), rep (t, 20), sep=":")
  C               <- as.factor(sample(rep(c("I","NI") , round(n / 2)), n))
  ...
  X               <- cbind(X, Loc_T)
  y <- X %*% b + rnorm(n)
  do.call(rbind, lapply(seq(1, 1.5, len =10), function(a){
  ...
  # Df <- apply(gfl2$mod$beta, 2, function(x){length(unique(x)) + 1})
  dj <- 4
  Df <- apply(Norm_Bet, 2, function(x){sum(x!=0) + (sum(x / gfl2$bOLSgr)*3) })
    #http://hansheng.gsm.pku.edu.cn/pdf/2008/agLasso.pdf
  BIC <- Error + Df*log(length(y)) / n
  b_hat <- gfl2$mod$beta
  b <- b[gfl2$ord]
  Roc <- t( apply(b_hat, 2, TP_FP, b=b) )
  colnames(Roc) <- c("TPR", "FPR")
  Roc <- cbind.data.frame(Roc, a = a, BIC =BIC)
...
ROC <- do.call(rbind, Simus)
 ...
 p <- ggplot (data = ROC, aes(x= FPR, y =BIC, color = as.character(a))) + geom_smooth()+
   theme_bw()
 p
