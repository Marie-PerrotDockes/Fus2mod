...
#   mod1 <- cv.glmnet(x = X2, y = response, standardize = FALSE,
#                    penalty.factor = c(0, 0, rep(b, (ncol(X) - 2)), rep(a, ncol(regressors))),
#                    intercept = F, lambda = lambda )
#
#   coef     <- coef(mod1, s="lambda.min")
#   reg_full <- names(coef[colnames(regressors),])[coef[colnames(regressors),] != 0]
#   reg_mod  <- colnames(X)[!(apply(sapply(reg_full,function(x){grepl(paste0(x,"$"),colnames(X))}), 1, sum)) |
#                               coef[colnames(X),] !=0 ]
#
#   mod <- stabsel(X2[,c(reg_mod,reg_full)], y, q=q, PFER=1, B = nrep,
#                  fitfun = glmnet.lasso,
