...
If we take a threshold of 0.8, we have a True Positive Rate equal to `r round(sum(sel %in% names(B_long[B_long!=0])) / sum(as.numeric(B_long)!=0),2)` and a False Positive Rate equal to `r round(sum(sel[-c(1,2)] %in% names(B_long[B_long==0])) / sum(as.numeric(B_long)==0),2)`.
